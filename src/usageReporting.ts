...
/**
 * Report token usage to VS Code Chat / Copilot.
 *
 * Prefers {@link vscode.LanguageModelUsagePart} when the host exposes it (proposed API PRs).
 * Otherwise emits the legacy `LanguageModelDataPart` with mime `usage` — the same shape
 * Copilot's `extChatEndpoint` already consumes on official builds.
 */
export function reportStreamUsage(
	...
	dialLog.info('reportStreamUsage → LanguageModelDataPart (mime=usage)', payload);
}
